...
public class MorseCodeConverter {
    ...
    private static final String[] LETTERS_STRING = new String[]{
            ".-",
            "-...",
            "-.-.",
            "-..",
    ...
    private static final String[] NUMBERS_STRING = new String[]{
            "-----",
            ".----",
            "..---",
            "...--",
    ...
    static String getCharacterCode(char c) {
        if (c >= 'A' && c <= 'Z') {
            return LETTERS_STRING[c - 'A'];
        }
        if (c >= 'a' && c <= 'z') {
            return LETTERS_STRING[c - 'a'];
        } else if (c >= '0' && c <= '9') {
            return NUMBERS_STRING[c - '0'];
        } else {
            return " ";
        }
    }

    static String getMorseCode(String text) {
        int length = text.length();
        StringBuilder code = new StringBuilder();
        for(int i = 0; i < length; ++i)
        {
            char c = text.charAt(i);
            code.append(getCharacterCode(c)).append(" ");
        }
